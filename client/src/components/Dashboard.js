...
const Dashboard = () => {
  const { grants, loading, error, refreshData, dashboardData } = useGrants({
    ...
    try {
      ...
      if (statusData && statusData.length > 0) {
        ...
        setScraperStatus({
          ...
          sites:
            latestJob.sites_searched ||
            (latestJob.search_report && latestJob.search_report.sites_searched) ||
            0,
          queries:
            latestJob.queries_attempted ||
            (latestJob.search_report && latestJob.search_report.queries_attempted) ||
            0,
          successful:
            latestJob.successful_queries ||
            (latestJob.search_report && latestJob.search_report.successful_queries) ||
            0,
        ...
        <div className="bg-blue-50 border-l-4 border-blue-400 p-4 mb-4">
          <div className="flex justify-between">
            <div className="flex">
              ...
              <div className="ml-3">
                <h3 className="text-sm font-medium text-blue-800">Discovering Grants</h3>
                <div className="mt-1 text-sm text-blue-700">
                  {scraperStatus.message ||
                    'Searching for grants that match your organization profile...'}
                </div>
              </div>
            </div>
            <div className="text-right text-sm text-blue-700">
              <div>
                <span className="font-semibold">{scraperStatus.sites}</span> sites searched
              </div>
              <div>
                <span className="font-semibold">
                  {scraperStatus.successful}/{scraperStatus.queries}
                </span>{' '}
                successful queries
              </div>
              <div>
                <span className="font-semibold">{scraperStatus.grants}</span> grants found
              </div>
            </div>
          </div>
        </div>
          ...
          <div className="bg-white overflow-hidden shadow rounded-lg">
            <div className="px-4 py-5 sm:p-6">
              <div className="flex items-center">
                ...
                <div className="ml-5 w-0 flex-1">
                  <dl>
                    <dt className="text-sm font-medium text-gray-500 truncate">Won Funding</dt>
                    <dd className="flex items-baseline">
                      <div className="text-2xl font-semibold text-gray-900">
          ...
          <div className="bg-white overflow-hidden shadow rounded-lg">
            <div className="px-4 py-5 sm:p-6">
              <div className="flex items-center">
                ...
                <div className="ml-5 w-0 flex-1">
                  <dl>
                    <dt className="text-sm font-medium text-gray-500 truncate">Submitted Grants</dt>
                    <dd className="flex items-baseline">
                      <div className="text-2xl font-semibold text-gray-900">
